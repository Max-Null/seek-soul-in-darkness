---
name: omo-reflect
description: 工作回顾——审查近期工作，发现重复模式，建议可复用的 skill、agent、命令或配置改进。适用于"从过去的会话中学习""优化重复工作流""识别哪些应该转化为可复用指令"。Review recent work, find repeated patterns, and suggest reusable skills, agents, commands, or config changes.
---

# Reflect

Reflect is a 主 agent-only workflow for learning from repeated work. It
...
Store session summaries in `$DSH_HOME/oh-my-opencode-slim/reflections/sessions/`.
...
2. Project-local guidance and memories, such as `AGENTS.md`, `.opencode/`,
   `.dsh/`, notes, checkpoints, task progress files, and codemaps.
...
- custom agents and their prompt guidance;
- custom commands;
- prompt overrides and append files;
- active project config preset, model routing, skills, and MCP permissions;
...
Recommended:
- Add a small main agent prompt rule for <workflow> because <evidence>.
