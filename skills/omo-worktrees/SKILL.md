---
name: omo-worktrees
description: Git Worktree 管理——为复杂、高风险或并行任务创建安全的隔离编码分支。Manage Git worktrees as safe isolated coding lanes for complex, risky, or parallel work.
---

# Worktrees 隔离编码协议

The `worktrees` skill provides an opinionated, safe protocol for
managing Git worktrees as isolated coding lanes. Its value is giving the 主 agent a consistent workflow for parallel tasks, risky experiments, integration review, and cleanup.

## Core Contract

This is a **main agent-only** workflow. Sub-agents such as 工匠
can be assigned tasks inside a worktree lane, but the
主 agent owns lane planning, branch/path selection, file ownership,
...
Before executing any Git mutation command, the 主 agent must observe the
following guards:
...
2. Formulate a branch name. Default to `oc-plus/<slug>` unless project/user conventions dictate otherwise.
