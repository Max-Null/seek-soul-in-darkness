---
name: omo-codemap
description: 代码库地图——为陌生仓库生成层级化代码地图。高成本操作，仅在明确要求代码库文档或首次仓库映射时使用。Generate comprehensive hierarchical codemaps for UNFAMILIAR repositories. Expensive operation.
---
...
Once all specific directories are mapped, the main agent must create or update the root `codemap.md`. This file serves as the **Master Entry Point** for any agent or human entering the repository.
...
|-----------|------------------------|--------------|
| `src/agents/` | Defines agent personalities (main agent, Explorer) and manages model routing. | [View Map](src/agents/codemap.md) |
