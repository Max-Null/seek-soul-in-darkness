...
- Generate and evolve change-aware codemap state artifacts (`.dsh/codemap.json`) and scaffold placeholders (`codemap.md`).
...
- Persistence model: JSON state at `.dsh/codemap.json` with `metadata`, `file_hashes`, and `folder_hashes`.
...
- `cmdInit()` computes include/exclude candidate sets using `selectFiles()` and writes:
  1) `.dsh/codemap.json` via `saveState()`
...
- `codemap` skill invocation path in SKILL workflow is explicit: Step 1 checks `.dsh/codemap.json` or `.dsh/cartography.json`, then Step 2/3 selects init or incremental path.
