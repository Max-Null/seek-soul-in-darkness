...
| 13 | **quick-toolbar 在壳里缺「插件中心」按钮** | 机理与实测见 §7 #20（`createToolbar` 内的壳标志是一次性快照，`__SSID_SHELL__` 晚于 `apply` 注入 → 该按钮恒走探测路径且必失败）。修法：壳标志改为每次读取（函数化），或让 `hideIfShell` 在标志到达时补一次 `renderBuiltins()`。**待用户定夺**——壳标题栏已有「插件中心」入口，修了等于在悬浮球里再添一个并列入口（与侧栏 `hide:false` 同理，属产品选择而非缺陷修复）。 | ⏳ 待定夺（2026-09-14 记） |
