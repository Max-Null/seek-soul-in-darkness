...
54. **「试一个插件的新版本」不要拿发版基准当试验田 —— 它留了专门的开关**（2026-09-27 实踩，我改完又回退）：fork 版打出来的安装包首启即崩（`@max-null/dsh-node-appearance: pending (waiting for service: settingsScope)`），查下来是 **template 对该包声明 npm `0.5.0`**，而 **npm 上的 0.5.0 是 0.1.5 内核适配**（`dsh.client.inject` 要 `dsh-client-ui-settings-plugins`）、**0.6.0 才是 0.1.7 适配**（要 `dsh-client-ui-plugin-manager`，09-26 23:43 已发布到 npm，`dist-tags.latest` 就是它）。于是我把 template 的 `0.5.0` 改成 `0.6.0` —— **跑门立刻报红**，`profile-sync` 说：`「@max-null/dsh-node-appearance」版本失配：B 落后于 A（A=0.6.0 B=0.5.0）—— 可预期的稳态，部署后会按 A 补上`。**后面那句才是重点**：门并没有拦我，它只是提示「这个差异会随下一次部署流到用户机器上」—— 也就是说那一改会**把 0.6.0 装进内核仍是 0.1.5 的安装版**，正是改动 30 已写明的崩法。**处置**：template 回退到 `0.5.0`（门随之恢复 7/7 绿），改用 **`prepare-ssid-plugins.ts` 的 `SSID_PROFILE_TEMPLATE_DIR`** 指向一份临时副本（`.ssid-build/template-060/`），只在这一次打包里生效。**判据**：**动发版基准前先问「这个改动会不会随下一次发版装到别人机器上」** —— 会，那它就不是「试验」而是「改形态」，必须连同内核版本一起走；只想验证某个插件能不能用，`SSID_PROFILE_TEMPLATE_DIR` 就是为这个留的口子。相关：坑 #48（同名同版本、内容不同）、坑 #50（改基准前先跑门）。
