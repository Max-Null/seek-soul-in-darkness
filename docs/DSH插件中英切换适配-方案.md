...
**分发决策（2026-08-18）**：不发布 npm——dsh-ssid-panels 跟随 SSiD 壳使用，单独安装无意义。分发走 SSiD 壳侧（壳把包打入 profile 依赖 / 预制插件清单），version 仅作本地标记，无需对外发布。仓库保持本地（无 git remote，不建 GitHub 仓库），随 SSiD 壳走。
