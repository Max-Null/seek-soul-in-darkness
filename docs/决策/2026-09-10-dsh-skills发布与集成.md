...
> 状态：**已交付**（0.1.2 · 2026-09-18）——0.1.0 起的适配工作已完成；0.1.2 起本包扩为**两类来源**（上游适配 8 + 思灵自创 2），见 §八
...
## 八、2026-09-18：0.1.2 与两类来源

本包从「DSH 官方 skill 的适配包」扩成了「思灵的技能库」——新增 **2 个思灵自创技能**，并把两类来源的迭代差异做成了可机检的标记。

| 项 | 内容 |
|---|---|
| 版本 | 0.1.1 → **0.1.2**（npm 已发布，payload `fileCount 25 → 29`） |
| 新增技能 | `dsh-session-forensics`（会话取证，通用）、`dsh-upstream-watch`（DSH 上游观察，**本工作区专用**） |
| 标记方式 | 每个技能的 `SOURCE.md` 开头声明「思灵自创 / 不是 DSH 官方 skill 的适配」，并带「迭代方式：**不跟随 DSH 迭代**」；README 增「技能的两个来源」一节 |
| 图书馆分类 | `classify()` 改为读 `SOURCE.md` 判定来源：**上游适配**（16 篇）/ **自创技能**（4 篇）——侧栏不再把两类混在一起（篇数含 SKILL.md 与 SOURCE.md，技能数仍是 8 + 2） |
| 三处声明 | `profile-template` / `ssid` / `web` 的 `dependencies` 均升 0.1.2（`dsh.profile.bundles` 条目本已存在，未动） |

**为什么不拆成两个库**：本包的分发路径是随 SSiD 走（`profile-template` 双处声明），用户不搜索也不挑选——拆库不会提升触达，反而要多维护一套脚手架（`build-index.mjs` + `verify-library.mjs` + 展示层；`verify-library.mjs:4` 自己写着「三个库同源，内容应逐字一致」）。真正需要的是**标记**，不是分家。

**自创技能的维护纪律**：它们**不跟随 DSH 迭代**——DSH 升级时只需重判那 8 个适配技能的差异；自创技能只在「本仓库路径 / 会话日志格式 / 默认值」变化时更新，触发条件写在各自的 `SOURCE.md` 里。

两个自创技能的来路：`dsh-session-forensics` 出自 `docs/排查/2026-09-18-长会话输出退化.md` 那条排查线（配套工具 `shell/scripts/session-degeneration-scan.mjs`）；`dsh-upstream-watch` 是工作区上游观察流程的固化。
