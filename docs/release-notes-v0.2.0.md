...
## 2026-09-07 重打包（同号 v0.2.0 更新，资产已替换）

- **原因**：v0.2.0 首发后插件继续升级（chat-rail / plugin-center 修复链、第三方更新），重新归档，
  确保新下载即含最新插件；已装用户启动时指纹不一致自动重部署（`e13b24f3`）。
- **插件基线（全部适配 DSH 0.1.2-rc.1，未引入 0.1.3 适配版）**：
  - `@max-null/dsh-chat-rail` 0.6.0 → **0.6.1**（TypeError this 绑定修复 + 跳转四件套：
    loaded 快路径/首条落位 clamp/加载中阶段信号/滚动条隐藏）
  - `@max-null/dsh-plugin-center` 0.2.17 → **0.2.19**（更新弹窗全量+已读态、本地日期节拍、
    changelog 多级降级：GitHub 限流缓存 / readme 预览 / 发布行、变更外链）
  - `dsh-genui` 0.9.7→**0.9.8**、`dsh-context` 0.42.0→**0.44.0**、`ds-harness-remote` 0.4.9→**0.4.10**、
    `dsh-session-manager` 0.4.4→**0.4.6**（npm 最新，rc.1 peer 兼容）
- **验证**：SSiD dev 实测通过（版本核对 + chat-rail 跳转 + 插件中心检查更新 + 入口冒烟，0 error）；
  归档抽查全绿（指纹/内核/插件版本/vendor 一致/无污染）。
- **mac 资产**：由仓库 build-mac CI（tag 触发）自动重打并上传，与本版同基线（含新插件）；
  本次重打包后 CI 已自动刷新（run 34051598046，success）。
