...
| 3 | 消息导航栏 | `@max-null/dsh-chat-rail` | 已发布 | ✅ | 画卷式展开 + scroll-spy 高亮居中 + 编号/时间/预览 + better-sidebar 双面板动画同步避让 |
| 4 | 记忆 UI 面板 | dsh-memory 扩展 | M1 | 🔲 | Typert remote + 设置页 tab（施工图已出） |
| 5 | Guardian 状态引擎 | `@max-null/dsh-guardian`（新） | M2 | 🔲 | 六触发线 → `session/event` 重做 |
| 6 | 四 agent 编排 | `ssid-agents`（preset） | M3 | 🔲 | 双星/工匠/参谋/军师 → preset + subagent |
| 7 | 14 技能包 | `ssid-skills` | M3 | 🔲 | 8 mxy + 6 omo → DSH skill |
| 8 | 品牌皮肤 | `ssid-skin` | M4 | 🔲 | 思灵主题（深渊/巨鲸之眼） |
| 9 | 自研壳 | `ssid-shell`（非 DSH 插件，Electron 壳） | M4 | 🔲 | 参考 anywhere-labs，不换皮 |
| 10 | 草稿润色 | `@max-null/dsh-polish`（新） | M3（可并行） | 🔲 | host 侧 `ctx.llm.stream`（flash、不带历史）+ client 侧输入框按钮 + `setDraft` 回填 |
| 11 | 文件预览面板 | `@max-null/dsh-preview`（新） | M4 后 | 🔲 | 多格式渲染（mammoth/xlsx/pdfjs/html/md）；DSH 原生仅图片（`ui-attachment` "Images only"），非图片预览全空白 |
| 12 | 划选内容发送 | dsh-preview 子能力 | M4 后 | 🔲 | 预览面板划选 → 发送，复用 `ui-input-trigger` 的 `ReferenceInsert` / `setDraft` |
| 13 | 零散面板 | 诊断/文件修改卡/上下文 | M4 后 | 🔲 | 逐项评估「原生有/自研」 |
...
| `@max-null/dsh-chat-rail` | 画卷式消息导航（scroll-spy 高亮居中 + 编号/时间/预览 + better-sidebar 同步避让） |
...
1. **自研 10 项**：3 个已发布（memory / chinese-thinking / chat-rail），7 个待开发（按 M1→M4 排期）。
