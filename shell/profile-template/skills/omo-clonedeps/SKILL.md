---
name: omo-clonedeps
description: 克隆依赖源码——将关键依赖源码拉到本地供 OpenCode 审查。适用于理解 SDK/框架内部行为、调试库实现细节等场景。普通 API/文档问题用联网查证即可，不需要此工具。Clone important dependency source code into local workspace for library internals inspection.
---
...
main agent does the repo-specific thinking and performs the approved
filesystem/git operations directly.

## Workflow

### Step 1: Check Existing State

First check whether `.dsh/clonedeps.json` exists.

If it exists:

1. Read it before asking librarian for a new plan.
2. Check whether each listed `path` exists under `.dsh/clonedeps/repos/`.
...
### Step 2: Research Dependency Sources

Research dependency discovery and source resolution via web search.
...
The main agent owns final approval. Before cloning:
...
```text
.dsh/clonedeps/repos/<safe-repo-name>/
```
...
4. Clone into a temporary directory under `.dsh/clonedeps/repos/`, then move it
...
Write `.dsh/clonedeps.json` so future agents know what exists:
  ...
  "dependencies": [
    {
      ...
      "ref": "v1.3.17",
      "path": ".dsh/clonedeps/repos/opencode-ai__opencode",
...
Do not add `.dsh/clonedeps.json` to `.gitignore`. It is small, reviewable
project metadata that can be committed. Only the cloned repository contents
under `.dsh/clonedeps/repos/` should be ignored.
...
`.dsh/clonedeps/repos/` for inspection. Do not edit these clones.

- `.dsh/clonedeps/repos/<safe-name>/` - `<repo>` at `<ref>`; <one sentence on
  why this source is useful>.
- `.dsh/clonedeps/repos/<safe-name-2>/` - `<repo>` at `<ref>`; <one sentence on
  why this source is useful>.
```

Also keep `.dsh/clonedeps.json` updated as the structured manifest, but do not
...
Ask before removing `.dsh/clonedeps.json` or the `AGENTS.md` section because
