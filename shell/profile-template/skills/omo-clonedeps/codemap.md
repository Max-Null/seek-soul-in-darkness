...
- **Read-only clones**: Dependencies are cloned into `.dsh/clonedeps/repos/` and should not be modified.
...
- **Agent-driven**: No runtime TS helpers (no `getClonedDepPath` utility). Agents resolve paths from `.dsh/clonedeps.json` if needed.
- **Configuration**: Uses `.dsh/clonedeps.json` in the project root to define which repositories are cloned and their checked-out refs.

## Flow
1. **Check existing state**: Read `.dsh/clonedeps.json` (if it exists). Check whether each listed `path` exists under `.dsh/clonedeps/repos/`.
...
4. **Clone sources**: Orchestrator runs git commands directly. Creates one folder per source under `.dsh/clonedeps/repos/<safe-repo-name>/`. Prefers shallow clones, pinned tags, HTTPS URLs.
5. **Write local state**: Writes `.dsh/clonedeps.json` with structured manifest (version, updatedAt, dependencies array).
...
- **Depends on**: Git CLI (for `git clone`, `git ls-remote`), `.dsh/clonedeps.json` manifest.
- **Outputs**: Local filesystem paths under `.dsh/clonedeps/repos/` for agent inspection.

## Notes
- Cloned repositories are read-only and should not be edited.
- The cache directory (`.dsh/clonedeps/repos/`) is in the **project directory**, not the user's home directory.
- This skill is for development and debugging; it does not affect runtime behavior.
- `.dsh/clonedeps.json` is small structured metadata that can be committed.
- Only `.dsh/clonedeps/repos/` is gitignored.
